...
# load variables
word_freqs = pk.load(open('output/word_freqs.pk', 'rb'))
x = pk.load(open('output/input_data.pk', 'rb'))
y = pk.load(open('output/target_data.pk', 'rb'))
...
finalDir, chptsDir, logsDir = createModelDirs(outDir,modelDir)
...
vocab_size = min(MAX_FEATURES, len(word_freqs)) + 4
word2index = {x[0]: i+4 for i, x in enumerate(word_freqs.most_common(MAX_FEATURES))}
word2index["PAD"] = 0
word2index["UNK"] = 1
word2index["GO"] = 2
word2index["EOS"] = 3
...
pk.dump(word2index, open(os.path.join(finalDir,'word2index.pk'),'wb'))
pk.dump(index2word, open(os.path.join(finalDir,'index2word.pk'),'wb'))
...
for i in range(len(x_new)):
    ...
    for word in x_new[i]:
        if word in word2index:
            seqs_x.append(word2index[word])
        else:
            seqs_x.append(word2index["UNK"]) # Replace words with low frequency with <UNK>
               
    # Target sequences IDs
    seqs_y_input = [word2index["GO"]] # Start of Sentence ID
    for word in y_new[i]:
        if word in word2index:
            seqs_y_input.append(word2index[word])
            seqs_y_target.append(word2index[word])
        else:
            # Replace words with low frequency with <UNK>
            seqs_y_input.append(word2index["UNK"])
            seqs_y_target.append(word2index["UNK"])
    seqs_y_target.append(word2index["EOS"]) # End of Sentece ID
...
y_target = np.empty((len(y_target_ids), target_len, vocab_size))
...
encoder_embed_layer = Embedding(vocab_size, embed_size, mask_zero=True)
...
decoder_embed_layer = Embedding(vocab_size, embed_size, mask_zero=True)
...
oracion = np.array([[78,530,858]])


# In[ ]:


decode_sequence(oracion, 100)
...
decode_sequence(np.array([[1,2,3,4,5]]), 100)
